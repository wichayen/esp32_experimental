...
bool relay10_1 = false;
bool relay10_2 = true;
bool relay10_3 = false;
bool relay10_4 = true;
...
void setup(){
  ...
  display.display();
	...
	node10.begin(10, SerialRS485);    // Modbus slave ID 10
	node10.preTransmission(preTransmission); //Callback for configuring RS-485 Transreceiver correctly
	node10.postTransmission(postTransmission);

	node1.begin(1, SerialRS485);    // Modbus slave ID 1
...
void loop() {
    ...
    node1.writeSingleCoil(0, relay1_1);   //  register number : 00001
    node1.writeSingleCoil(1, relay1_2);
    node1.writeSingleCoil(2, relay1_3);
    node1.writeSingleCoil(3, relay1_4);
    relay1_1 = !relay1_1;
    relay1_2 = !relay1_2;
    relay1_3 = !relay1_3;
    relay1_4 = !relay1_4;
    ...
    result = node1.readCoils(0, 8);    //  Relay module support only 1 or 8 ?
    if(result == node1.ku8MBSuccess){
      ...
      Serial.println("node1 readCoils failed");
    }
    
	result = node1.readDiscreteInputs(0, 8);     //  Relay module support only 1 or 8 ?
    if(result == node1.ku8MBSuccess)
    {
      Serial.print("Input = ");
      ...
      Serial.println("node1 readDiscreteInputs failed");
    }
    ...
    node10.writeSingleCoil(0, relay10_1);   //  register number : 00001
    node10.writeSingleCoil(1, relay10_2);
    node10.writeSingleCoil(2, relay10_3);
    node10.writeSingleCoil(3, relay10_4);
    relay10_1 = !relay10_1;
    relay10_2 = !relay10_2;
    relay10_3 = !relay10_3;
    relay10_4 = !relay10_4;
    
    result = node10.readCoils(0, 10);
    if(result == node10.ku8MBSuccess){
      Serial.print("Coil = ");
      Serial.print((node10.getResponseBuffer(0)>>0)&0x01);
      Serial.print(" , ");
      Serial.print((node10.getResponseBuffer(0)>>1)&0x01);
      Serial.print(" , ");
      Serial.print((node10.getResponseBuffer(0)>>2)&0x01);
      Serial.print(" , ");
      Serial.println((node10.getResponseBuffer(0)>>3)&0x01);
    }else{
      Serial.println("node10 readCoils failed");
    }
    
    result = node10.readDiscreteInputs(0x0000, 10); 
    if(result == node10.ku8MBSuccess)
    {
      Serial.print("Input = ");
      Serial.print((node10.getResponseBuffer(0)>>0)&0x01);      //  register number : 10001
      Serial.print(" , ");
      Serial.print((node10.getResponseBuffer(0)>>1)&0x01);
      Serial.print(" , ");
      Serial.print((node10.getResponseBuffer(0)>>2)&0x01);
      ...
      Serial.println("node10 readDiscreteInputs failed");
    }
    
    result = node10.readInputRegisters(0x0000, 10); 
    if(result == node10.ku8MBSuccess)
    {
      Serial.print("Input register = ");
      Serial.print(node10.getResponseBuffer(0));        //  register number : 30001
      Serial.print(" , ");
      Serial.print(node10.getResponseBuffer(1));
      Serial.print(" , ");
      Serial.print(node10.getResponseBuffer(2));
      Serial.print(" , ");
      Serial.println(node10.getResponseBuffer(3));
    }else{
      Serial.println("node10 readInputRegisters failed");
    }
    
    
    node10.writeSingleRegister(0, time_1);       //  register number : 40001
    node10.writeSingleRegister(1, time_1+1);
    node10.writeSingleRegister(2, time_1+2);
    node10.writeSingleRegister(3, time_1+3);

    result = node10.readHoldingRegisters(0x0000, 10); 
    if(result == node10.ku8MBSuccess)
    {
      Serial.print("Holding register = ");
      Serial.print(node10.getResponseBuffer(0));
      Serial.print(" , ");
      Serial.print(node10.getResponseBuffer(1));
      Serial.print(" , ");
      Serial.print(node10.getResponseBuffer(2));
      ...
      Serial.println("node10 readHoldingRegisters failed");
    }
